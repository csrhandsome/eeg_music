...
def arduino_play_by_scale():
    ...
    webserver = WebServer(arduino_reader=reader)
    # 连接Arduino
    if reader.connect():
        try:
                ...
                if reader.serial and reader.serial.in_waiting:
                    ...
                    try:
                        ...
                        if line and line != last_processed_line:
                            ...
                            # 获取当前数据
                            current_data = reader.current_data
...
def arduino_play_by_rate():
    ...
    parser.add_argument('-m', '--max-sounds', type=int, default=100, help='最大同时存在的声音对象数量')
    parser.add_argument('--no-music', action='store_true', help='不播放音乐,只运行WebSocket服务器')
    args = parser.parse_args()
                ...
                if arduino_reader.serial and arduino_reader.serial.in_waiting:
                    ...
                    try:
                        ...
                        if line and line != last_processed_line:
                            ...
                            # 获取当前数据
                            data = arduino_reader.current_data
